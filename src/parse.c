...
uint8_t process_trigger_smart_contract_data_v2_for_multi(pb_istream_t *stream, txContent_t *content, bool init) {
    if (init)
    {
        // If handling the beginning of the data field, assume that the function selector is
        // present
...
void copyContract(uint32_t tag_last, uint32_t tag_now, txContent_t *content, protocol_Transaction_Contract *contract) {
    switch (tag_now)
    {
...
bool packageLeftForNext(uint8_t *buffer, uint32_t length, txContent_t *content, uint8_t bytes_left) {
    ...
    content->tokenNamesLength[0] = bytes_left;
    memmove(content->tokenNames[0], buffer+(length-content->tokenNamesLength[0]), content->tokenNamesLength[0]);
    return true;
}
...
bool mergeLeftPackToBuffer(txContent_t *content, uint8_t *buffer, uint32_t *length) {
    if (content->tokenNamesLength[0] > 0)
    {
        if (*length + content->tokenNamesLength[0] > 255)
        {
            ...
            memmove(content->tokenNames[1], buffer+(*length-content->tokenNamesLength[1]), content->tokenNamesLength[1]);
            *length -= content->tokenNamesLength[1];
        }
        if (!moveBuffer(buffer, *length-content->tokenNamesLength[0], content->tokenNamesLength[0]))
        {
            return false;
        }
    ...
    if (content->tokenNamesLength[1] > 0)
    {
        if (content->tokenNamesLength[0] > 0)
        {
            *length = content->tokenNamesLength[0] + content->tokenNamesLength[1];
            ...
            memmove(buffer, content->tokenNames[0], content->tokenNamesLength[0]);
            content->tokenNamesLength[0] = 0;
            memmove(buffer+content->tokenNamesLength[0], content->tokenNames[1], content->tokenNamesLength[1]);
            content->tokenNamesLength[1] = 0;
        } else {
            *length = content->tokenNamesLength[1];
...
uint32_t getLeftForLevel(txContent_t *content, uint8_t level, uint32_t length) {
    ...
    case 4: // transaction_raw.contract.parameter.value.(TriggerSmartContract)data level
        return length >= (uint32_t)content->exchangeID ? (uint32_t)content->exchangeID:length;
    default:
        return 0;
    }
    return 0;
...
uint32_t processTxForCSMulti(uint8_t *buffer, uint32_t length, txContent_t *content) {
    ...
    if (!pb_decode_contract(&stream, protocol_Transaction_raw_fields, &transaction, &tag))
    {
        ...
        if (tag == protocol_Transaction_raw_custom_data_tag)
        {
            ...
            uint32_t tar_size;
            if (!decode_field_for_contract(&stream, protocol_Transaction_raw_fields, &transaction, &tar_size)) {
                // return USTREAM_FAULT;
                return 8;
            }
        ...
        if (tag == protocol_Transaction_raw_contract_tag)
        {
            ...
            if (!pb_decode_contract(&stream, protocol_Transaction_Contract_fields, &contract, &tag)) {
                ...
                if (decode_tag(&stream, &tag))
                {
                    ...
                    if (tag > protocol_Transaction_Contract_parameter_tag) {
                        ...
                        if (!decode_field_for_contract(&stream, protocol_Transaction_Contract_fields, &contract, &tar_size)) {
                            // return USTREAM_FAULT;
                            return 11;
                        }
                        ...
                        if (!pb_decode_contract(&stream, google_protobuf_Any_fields, &anyValue, &tag)) {
                            ...
                            if (decode_tag(&stream, &tag))
                            {
                                ...
                                if (tag == google_protobuf_Any_value_tag)
                                {
                                    ...
                                    if (!decode_field_for_contract(&stream, google_protobuf_Any_fields, &anyValue, &tar_size)) {
                                        // return USTREAM_FAULT;
                                        return 12;
                                    }
                                    ...
                                    if (!pb_decode_contract(&stream, protocol_TriggerSmartContract_fields, &msg.trigger_smart_contract, &tag)) {
                                        ...
                                        if (decode_tag(&stream, &tag))
                                        {
                                            ...
                                            if (tag == protocol_TriggerSmartContract_data_tag) {
                                                ...
                                                if (!decode_field_for_contract(&stream, protocol_TriggerSmartContract_fields, &msg.trigger_smart_contract, &tar_size)) {
                                                    // return USTREAM_FAULT;
                                                    return 12;
                                                }
                                                ...
                                                if (process_trigger_smart_contract_data_v2_for_multi(&stream, content, true) < 7)
                                                {
                                                    // return USTREAM_FAULT;
                                                    return 21;
                                                }
            ...
            if (transaction.contract->has_parameter) {
                copyContract(0, protocol_Transaction_Contract_Permission_id_tag+1, content, transaction.contract);

                pb_istream_t tx_stream = pb_istream_from_buffer(contract_buffer.buf, contract_buffer.size);
                if (transaction.contract->type != protocol_Transaction_Contract_ContractType_TriggerSmartContract)
                {
                    // return USTREAM_FAULT;
                    return 19;
                }
...
uint8_t processTriggerSmartContractData(uint8_t *buffer, uint32_t length, txContent_t *content, bool init){
    pb_istream_t stream = pb_istream_from_buffer(buffer, length);
    initTargetSize(content, 10, stream.bytes_left);
    ...
    if (!pb_decode_contract(&stream, protocol_TriggerSmartContract_fields, &msg.trigger_smart_contract, &tag)) {
        ...
        if (decode_tag(&stream, &tag))
        {
            ...
            if (tag == protocol_TriggerSmartContract_data_tag) {
                ...
                uint32_t subLength2 = stream.bytes_left >= content->exchangeID ? content->exchangeID:stream.bytes_left;
        ...
        if (tag_last < protocol_TriggerSmartContract_data_tag && tag >= protocol_TriggerSmartContract_data_tag)
        {
            pb_istream_t tx_stream = pb_istream_from_buffer(contract_buffer.buf, contract_buffer.size);
            if (!process_trigger_smart_contract_data_v2(&tx_stream, content)) {
                // return USTREAM_FAULT;
    ...
    if (!pb_decode_contract(&stream, google_protobuf_Any_fields, &anyValue, &tag)) {
        ...
        if (decode_tag(&stream, &tag))
        {
            ...
            if (tag == google_protobuf_Any_value_tag)
            {
                ...
                return decodeTriggerSmartContract(buffer+(length-stream.bytes_left), stream.bytes_left, content);
            } else {
                if (stream.bytes_left > 200)
                {
                    // return USTREAM_FAULT;
...
uint32_t processTxForCSMultiParts(uint8_t *buffer, uint32_t length, txContent_t *content, uint8_t state) {
    ...
    if (state == 0x90)
    {
        ...
        if (content->tokenNamesLength[1] > 0)
        {
            return processTxForCSPart((uint8_t *)content->tokenNames[1], content->tokenNamesLength[1], content, state);
        }
    }
    return txResult;
}
