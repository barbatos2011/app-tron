                ...
                case 1:
                    if (types[i] == TYPE_ACCOUNT) {
                        ...
                        if ((chainConfig->chainId != *chain_id) &&
                            (memcmp(addr, g_trusted_name_info.addr, ADDRESS_SIZE_712) == 0)) {
                            ret = true;
                        }
                    }
                    break;
...
static bool handle_not_valid_after(const s_tlv_data *data,
    ...
    for (; i < (int) ARRAYLEN(app_version); ++i) {
        if (data->value[i] > app_version[i]) break;
...
static bool handle_address(const s_tlv_data *data,
                           ...
                           s_sig_ctx *sig_ctx) {
    (void) sig_ctx;
    if (data->length != ADDRESS_SIZE_712) {
        return false;
    }
    memcpy(trusted_name_info->addr, data->value, ADDRESS_SIZE_712);
    return true;
}
 ...
 */
uint16_t handle_provide_trusted_name(uint8_t p1, uint8_t p2, const uint8_t *data, uint8_t length) {
    UNUSED(p2);
    s_sig_ctx sig_ctx;
